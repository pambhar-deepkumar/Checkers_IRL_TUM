...
def main():
    config = read_config()
    boards_list, metrics, winning = generate_game_states(1000)
    metrics_model, history = train_metrics_model(metrics = metrics[:1000], winning = winning[:1000], save_path=config["paths"]['metrics_model_save_path'])
